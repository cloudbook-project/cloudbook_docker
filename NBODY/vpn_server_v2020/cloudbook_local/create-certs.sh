...
[alt_names]
IP.1  = 10.10.0.1
DNS.1 = server.cloudbook_project.es
EOF
...
openssl req \
  ...
  -nodes \
  -x509 \
  -subj "/C=ES/ST=Experis/L=Cloudbook/O=Cloudbook CA" \
  -keyout "${DIR}/ca.key" \
  -out "${DIR}/ca.crt"
#
...
# the bits to 4096.
openssl genrsa -out "${DIR}/cloudbook-server.key" 2048

# Generate a CSR using the configuration and the key just generated. We will
# give this CSR to our CA to sign.
openssl req \
  -new -key "${DIR}/cloudbook-server.key" \
  -out "${DIR}/cloudbook-server.csr" \
...
openssl x509 \
  -req \
  -days 120 \
  -in "${DIR}/cloudbook-server.csr" \
  -CA "${DIR}/ca.crt" \
  -CAkey "${DIR}/ca.key" \
  -CAcreateserial \
  -extensions v3_req \
  -extfile "${DIR}/openssl.cnf" \
  -out "${DIR}/cloudbook-server.crt"

# (Optional) Verify the certificate.
openssl x509 -in "${DIR}/cloudbook-server.crt" -noout -text
